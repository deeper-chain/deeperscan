...
class PolkascanHarvesterService(BaseService):
    ...
    def process_metadata(self, spec_version, block_hash):
        ...
        if runtime:
            ...
            logger.info('missing runtime version {} in db'.format(spec_version))
            ...
            try:
                ...
                for module_index, module in enumerate(metadata_decoder.pallets):
                    ...
                    if len(module.calls or []) > 0:
                        for idx, call in enumerate(module.calls):
                            ...
                            self.db_session.add(runtime_call)
                    ...
                    if len(module.events or []) > 0:
                        for event_index, event in enumerate(module.events):
                            ...
                            self.db_session.add(runtime_event)
                            self.db_session.flush()
                            logger.debug('save runtime event, id={}, spec_version={}, module_id={}, event_id={}'.format(runtime_event.id, spec_version, module_id, event.name))
                ...
                logger.error('Could not store runtime version {} to db'.format(spec_version), exc_info=True)
    ...
    def add_block(self, block_hash):
        ...
        try:
            ...
            RuntimeConfiguration().set_active_spec_version_id(int(parent_spec_version))
            events_decoder = self.substrate.get_events(block_hash)

            # Revert back to current runtime
            RuntimeConfiguration().set_active_spec_version_id(int(block.spec_version_id))
