...
pipeline {
    ...
    stages {
        ...
        stage('Build') {
            ...
            dir("explorer-api"){
                sh 'echo $TAG ...........'
                sh 'docker build --no-cache -t 561108432312.dkr.ecr.ap-southeast-1.amazonaws.com/deeperscan/pre-explorer-api:$TAG .'
            }
            dir("harvester"){
                sh 'docker build --no-cache -t 561108432312.dkr.ecr.ap-southeast-1.amazonaws.com/deeperscan/pre-harvester:$TAG .'
            }
            dir("explorer-gui"){
                sh 'docker build --no-cache --build-arg API_URL=https://dev.deeperscan.io/api/v1 --build-arg NETWORK_NAME=Deeper --build-arg NETWORK_ID=deeper --build-arg NETWORK_TYPE=pre --build-arg CHAIN_TYPE=relay --build-arg NETWORK_TOKEN_SYMBOL=DPR --build-arg NETWORK_TOKEN_DECIMALS=18 --build-arg NETWORK_COLOR_CODE=21C355 -t 561108432312.dkr.ecr.ap-southeast-1.amazonaws.com/deeperscan/pre-explorer-gui:dev-$TAG .'
                sh 'docker build --no-cache --build-arg API_URL=https://scan.deeper.network/api/v1  --build-arg NETWORK_NAME=Deeper --build-arg NETWORK_ID=deeper --build-arg NETWORK_TYPE=pre --build-arg CHAIN_TYPE=relay --build-arg NETWORK_TOKEN_SYMBOL=DPR --build-arg NETWORK_TOKEN_DECIMALS=18 --build-arg NETWORK_COLOR_CODE=21C355 -t 561108432312.dkr.ecr.ap-southeast-1.amazonaws.com/deeperscan/pre-explorer-gui:prod-$TAG .'
            }
            }
        }
        stage('Push Image'){
